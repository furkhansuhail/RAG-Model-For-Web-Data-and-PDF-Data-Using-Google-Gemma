...
#     def LLM_Model_Setup(self):
#         chunks, embeddings = self.load_combined_embeddings()
#         print("🔗 Loaded combined embeddings")
#         print("📏 Embeddings shape:", embeddings.shape)
#
#         use_quant = self.quantization_configuration_setup()
#
#         try:
#             if torch.cuda.get_device_capability(0)[0] >= 8:
#                 import flash_attn
...
#
#         print(f"✅ Model loaded: {self.model_id}")
#         print(f"🧠 Num params: {self.get_model_num_params(self.llm_model)}")
#         print(f"💾 Memory footprint: {self.get_model_mem_size(self.llm_model)}")
#
#         # 💡 This returns a list of top results
#         results = self.LLM_PDF_WEB_Query_Search(self.save_path_Combined_Embeddings, self.topic)
#
#         # ✅ Fix: Return results with search method string
#         return results, "LLM"
#
#
#
#     def LLM_PDF_WEB_Query_Search(self, embeddings_path, query, similarity_method="llm"):
#         chunks, embeddings = self.load_combined_embeddings()
#         if not chunks or embeddings.numel() == 0:
#             print("❌ No embeddings available.")
#             return
#
#         results = self.get_top_results(chunks, embeddings, query, similarity_method)
#         print("🧠 Top Results:")
#         for r in results:
#             print(f"- {r['source']}: {r['text'][:100]}...")
#
#         return results
#
#     def get_top_results(self, chunks, embeddings, query, mode):
#         if mode != "llm":
#             print(f"❌ Unsupported mode: {mode}")
#             return []
#
#         scores, indices = self.retrieve_relevant_resources(query=query, embeddings=embeddings)
#         results = []
#         for i, score in zip(indices, scores):
#             chunk = chunks[i]
#             results.append({
...
#
#         input_ids = self.tokenizer(prompt, return_tensors="pt").to(self.device)
#         outputs = self.llm_model.generate(**input_ids, temperature=0.7, do_sample=True, max_new_tokens=2048)
...
# User query: {query}
# Answer:"""
#         base_prompt = prompt_template.format(context=context, query=query)
#         chat_format = [{"role": "user", "content": base_prompt}]
#         prompt = self.tokenizer.apply_chat_template(chat_format, tokenize=False, add_generation_prompt=True)
#         return prompt
#
